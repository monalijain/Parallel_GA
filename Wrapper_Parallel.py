...
def processIndividual(q):
    while not exitFlag:
        ...
        if not workQueue.empty():
            ...
            Make_Performance_Measures(TBeginList[walkNo],TendList[walkNo],Train_Or_Report="Training",walkforward_number=walkforward_number,dbObject=dbObject)
            Make_Performance_Measures(RBeginList[walkNo],REndList[walkNo],Train_Or_Report="Reporting",walkforward_number=walkforward_number,dbObject=dbObject)
...
#making a database object
    ...
    Number=dbObject.dbQuery("SELECT COUNT(DISTINCT(individualid)),1 FROM "+gv.name_Tradesheet_Table)

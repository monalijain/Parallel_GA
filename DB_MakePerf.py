...
def Make_Performance_Measures(Begin,End,Train_Or_Report,walkforward_number,dbObject):
    ...
    resultDates = dbObject.dbQuery("SELECT DISTINCT(date),1 FROM price_series_table WHERE date >= '" + str(BeginDate.date())+ "' AND date <= '"+str(EndDate.date())+"'")
    #dbObject2.dbClose()
    ...
    resultIndividuals = dbObject.dbQuery(query)
...
# dateinrange=CurrentDate.date()<=EndDate.date() and CurrentDate.date()>=BeginDate.date()
    ...
    #calculation of DD:
            ...
            dbObject.dbQuery("INSERT INTO "+ string_1+""
